...
final case class Facet(v1: Vertex, v2: Vertex, v3: Vertex, colorOpt: Option[java.awt.Color] = None) {
  ...
  def scaled(x: Double = 1, y: Double = 1, z: Double = 1): Facet = copy(
    v1 = v1.scaled(x, y, z),
    v2 = v2.scaled(x, y, z),
    v3 = v3.scaled(x, y, z)
  )

  def rotated(x: Double = 0, y: Double = 0, z: Double = 0): Facet = copy(
    v1 = v1.rotated(x, y, z),
    v2 = v2.rotated(x, y, z),
    v3 = v3.rotated(x, y, z)
  )

  def moved(x: Double = 0, y: Double = 0, z: Double = 0): Facet = copy(
    v1 = v1.moved(x, y, z),
    v2 = v2.moved(x, y, z),
    v3 = v3.moved(x, y, z)
  )

  def flip: Facet = copy(v1 = v1, v2 = v3, v3 = v2)

  def area: Double = (v2 - v1).length * (v3 - v1).length * 0.5
}
